...
import { AnalysisResult, ComplexityRating, Issue } from '@/lib/types';
...
import { VERSION, PRISM_LANGUAGE_MAP } from '@/lib/constants';
...
function MetricCard({ title, value, rating, description, icon, isDark, className = '' }: {
  ...
  return (
    <div className={`rounded-xl p-4 border transition-colors flex flex-col ${isDark ? 'bg-[#111828] border-slate-700/50' : 'bg-white border-slate-200'} ${className}`}>
      ...
      <div className={`text-2xl md:text-3xl font-bold font-mono mb-1 ${c.text} tracking-tight break-all`}>{value}</div>
              ...
              {({ className, tokens, getLineProps, getTokenProps }) => (
                <pre className={`${className} bg-transparent pb-5 m-0 text-[13px] leading-6 font-mono`} style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                  {tokens.map((line, i) => {
                    ...
                    const { key: _lineKey, ...safeLineProps } = lineProps;
                    return (
                      <div key={i} {...safeLineProps} className={`grid grid-cols-[3rem_1fr] items-start ${isAffected ? (isDark ? 'bg-rose-500/10' : 'bg-rose-50') : ''}`}>
                        ...
                        <div className="font-mono leading-6 whitespace-pre-wrap break-words">
                          {line.map((token, key) => {
                            ...
                            const { key: _tokenKey, ...safeTokenProps } = tokenProps;
                            return <span key={key} {...safeTokenProps} />;
                          })}
                        </div>
                      </div>
                    );
                ...
                {({ className, tokens, getLineProps, getTokenProps }) => (
                  <pre className={`${className} bg-transparent pb-5 m-0 text-[13px] leading-6 font-mono`} style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                    {tokens.map((line, i) => {
                      ...
                      const { key: _lineKey, ...safeLineProps } = lineProps;
                      return (
                        <div key={i} {...safeLineProps} className="grid grid-cols-[3rem_1fr] items-start border-l-2 border-transparent">
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const tooltipFormatter = ((value: number | string, name: string) => {
    ...
    return [v, name];
  }) as unknown as (value: number | string, name: string) => [string, string];

  const tooltipLabelFormatter = (_: unknown, payload: ReadonlyArray<{ payload: { originalN: number } }>) => {
    ...
    <div className={`min-h-screen ${bg} ${isDark ? 'text-slate-200' : 'text-slate-800'} font-sans px-4 py-6`}>
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3 min-w-0">
            <button
              ...
              className={`p-1.5 rounded-md transition-colors shrink-0 ${isDark ? 'text-slate-400 hover:bg-slate-800 hover:text-white' : 'text-slate-500 hover:bg-slate-200 hover:text-slate-900'}`}
              aria-label="Back to editor"
            >
              <ArrowLeft size={18} />
            </button>
            <span className={`font-black tracking-tighter kode-font text-[24px] sm:text-[32px] truncate ${textPrimary}`}>
              CODALYZER<span className="text-blue-500 text-[14px] sm:text-[16px] font-mono ml-2.5 opacity-70">// v{VERSION}</span>
            </span>
          </div>
          <button onClick={toggleTheme} className={`flex items-center justify-center w-9 h-9 rounded-lg transition-colors border shrink-0 ${isDark ? 'bg-slate-800/50 border-slate-700 text-slate-400 hover:text-white' : 'bg-white border-slate-200 text-slate-500 hover:text-slate-900'}`} aria-label="Toggle theme">
            {isDark ? <Sun size={18} /> : <Moon size={18} />}
          </button>
        </div>

        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <h1 className={`text-2xl sm:text-3xl md:text-4xl font-bold space-font ${textPrimary} break-all`}>
            Report:<span className="text-blue-500 ml-3">{result.fileName}</span>
          </h1>
          ...
          <div className="flex gap-3 flex-wrap items-center">
            {shareUrl ? (
              <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                ...
                />
                <button onClick={handleCopy} className="text-blue-500 hover:text-blue-400 transition-colors" aria-label="Copy share link">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-3 mb-6 relative items-stretch">
        <div className="lg:col-span-3 relative flex flex-col">
          ...
          <div className={`rounded-xl border transition-colors overflow-hidden flex-1 flex flex-col ${isDark ? 'bg-[#111828] border-slate-700/50' : 'bg-white border-slate-200'}`}>
            <div className={`grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 flex-1 ${isDark ? 'divide-slate-700/50' : 'divide-slate-200'}`}>
              <div className="p-4 flex flex-col flex-1 relative">
                <div className="flex justify-between items-start mb-2">
                  <span className={`text-xs font-bold uppercase tracking-widest ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Best Case</span>
                  <CheckCircle size={16} className="text-blue-500" />
                </div>
                <div className={`text-2xl md:text-3xl font-bold font-mono mb-1 tracking-tight break-all ${result.timeComplexity.best.rating === ComplexityRating.Good ? 'text-blue-500' : 'text-yellow-400'}`}>
                  {result.timeComplexity.best.notation}
                </div>
              ...
              <div className="p-4 flex flex-col flex-1 relative">
                ...
                <div className={`text-2xl md:text-3xl font-bold font-mono mb-1 tracking-tight break-all ${result.timeComplexity.average.rating === ComplexityRating.Good ? 'text-blue-500' : 'text-yellow-400'}`}>
                  {result.timeComplexity.average.notation}
                </div>
              ...
              <div className="p-4 flex flex-col flex-1">
                ...
                <div className={`text-2xl md:text-3xl font-bold font-mono mb-1 tracking-tight break-all ${result.timeComplexity.worst.rating === ComplexityRating.Good ? 'text-blue-500' :
        ...
        <div className="lg:col-span-1 flex flex-col">
          ...
          <MetricCard
            ...
            description={result.spaceComplexity.description}
            ...
            <div className="h-[280px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={timeChartData} margin={{ top: 10, right: 10, left: 0, bottom: 25 }}>
                  ...
                  <XAxis
                    dataKey="displayN" stroke={chartTick}
                    tick={{ fontSize: 12, fill: chartTick }} tickLine={false} axisLine={false}
                  ...
                  />
                  <YAxis
                    dataKey="ops" stroke={chartTick}
                    tick={{ fontSize: 12, fill: chartTick }} tickLine={false} axisLine={false} width={60} allowDecimals={false}
                  ...
                  />
                  <Area
                    type="monotone" dataKey="ops" name="Operations" stroke="#3b82f6" strokeWidth={2.5} fill="url(#colorOps)"
            ...
            <div className="h-[280px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={spaceChartData} margin={{ top: 10, right: 10, left: 0, bottom: 25 }}>
                  ...
                  />
                  <YAxis
                    dataKey="memory" stroke={chartTick}
                    tick={{ fontSize: 12, fill: chartTick }} tickLine={false} axisLine={false} width={60} allowDecimals={false}
                  ...
                  />
                  <Area
                    type="monotone" dataKey="memory" name="Memory" stroke="#22d3ee" strokeWidth={2.5} fill="url(#colorMemory)"
      ...
      <div className="grid grid-cols-1 gap-4 mb-6">
        {result.issues.length > 0 ? (
          result.issues.map((issue) => (
            <IssueCard key={issue.id} issue={issue} isDark={isDark} sourceCode={result.sourceCode} language={result.language} />
          ))
        ) : (
      ...
      <footer className="mt-12 pb-6 text-center">
        ...
        <div className={`flex flex-wrap items-center justify-center gap-3 kode-font text-[11px] sm:text-[12px] font-black uppercase tracking-[0.3em] ${isDark ? 'text-white/60' : 'text-slate-800'}`}>

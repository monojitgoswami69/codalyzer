        ...
        <div className="text-center mb-6">
          <div className="flex items-center justify-center gap-2 mb-3">
            ...
            <h1 className={`text-lg font-bold tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>
              Codalyzer{' '}
              <span className="text-blue-500 text-[10px] font-mono font-normal ml-0.5 opacity-70">// v{VERSION}</span>
            </h1>
          </div>
        </div>
        ...
        <div className={`rounded-lg p-5 border ${isDark ? 'bg-slate-800/50 border-slate-800' : 'bg-white border-slate-200'}`}>
          ...
          <div className={`border-t pt-4 ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
            <p className={`text-xs text-center leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              Please open this page on a{' '}
              <strong className={isDark ? 'text-white' : 'text-slate-900'}>desktop or laptop computer</strong> with a screen width of at least{' '}
              <strong className={isDark ? 'text-white' : 'text-slate-900'}>1024px</strong>.
            </p>
          </div>
        </div>
      </div>

...
export const LandingPage: React.FC<LandingPageProps> = ({ onGetStarted }) => {
  ...
  const getTransform = () => (isExiting ? 'translate-x-full' : 'translate-x-0');
        ...
        <main className="flex-1 container mx-auto px-6 flex flex-col">
          <div className="flex-1 flex flex-col items-center justify-center animate-fade-in">
            <div className="max-w-4xl w-full text-center">
              <h1 className={`text-5xl sm:text-6xl md:text-8xl font-black quantico-font tracking-tighter mb-8 leading-[0.9] ${isDark ? 'text-white' : 'text-slate-900'}`}>
                WELCOME TO <br />
              ...
              <p className={`text-base sm:text-xl md:text-2xl mb-12 max-w-2xl mx-auto leading-relaxed font-light ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                The standard for high-performance{' '}
                <span className={`font-medium ${isDark ? 'text-white' : 'text-slate-900'}`}>complexity detection</span>.

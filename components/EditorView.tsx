    ...
    <div className={`flex flex-col h-screen ${bg} text-slate-300 overflow-hidden`}>
      <header className={`h-14 flex items-center justify-between px-4 ${isDark ? 'bg-[#181821]' : 'bg-[#DBDFE7]'} z-20 shadow-sm border-b ${isDark ? 'border-slate-800/50' : 'border-slate-300/50'}`}>
        <div className="flex items-center gap-3">
          <span className={`font-black tracking-tighter kode-font text-[20px] sm:text-[28px] ${textPrimary} select-none`}>
            CODALYZER<span className="text-blue-500 text-[12px] sm:text-[14px] font-mono ml-2 opacity-70">// v{VERSION}</span>
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              {hasValidReport && (
                <button onClick={onViewReport} className="flex items-center gap-1.5 px-3 h-7 rounded-md text-[11px] font-bold tracking-tight transition-all bg-green-600 hover:bg-green-500 text-white jetbrains-font">
                  <Eye size={12} /> <span className="hidden sm:inline">VIEW ANALYSIS</span><span className="sm:hidden">VIEW</span>
                </button>
              )}
              <button
          ...
          <button onClick={toggleTheme} className={`flex items-center justify-center w-7 h-7 rounded-md transition-colors border ${isDark ? 'bg-slate-800/50 border-slate-700 text-slate-400 hover:text-white' : 'bg-white border-slate-200 text-slate-500 hover:text-slate-900'}`} aria-label="Toggle theme">
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        <div className={`w-56 lg:w-64 flex flex-col ${bg} shrink-0`}>
          <div className="px-2 pt-4 pb-2">
            <div className="flex gap-2">
              ...
              <button onClick={() => fileInputRef.current?.click()} className={`flex items-center justify-center p-2 rounded border transition-colors ${isDark ? 'bg-slate-800 hover:bg-slate-700 text-slate-300 border-slate-700' : 'bg-slate-100 hover:bg-slate-200 text-slate-600 border-slate-300'}`} aria-label="Upload file">
                <Upload size={14} />
              </button>
              ...
              <div className="space-y-0.5">
                {files.map((file) => (
                  <div
                    ...
                    className={`group flex items-center justify-between px-3 py-2 rounded cursor-pointer transition-colors ${file.id === activeFileId
                      ? isDark
          ...
          <div className="p-3">
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 px-3 py-2 rounded text-xs transition-colors ${isDark ? 'text-slate-400 hover:bg-slate-800 hover:text-slate-300' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-700'}`}
            >
      ...
      <div className={`h-8 flex items-center justify-between px-2 text-[11px] sm:text-[12px] kode-font font-black ${isDark ? 'bg-[#181821] text-white/70' : 'bg-[#DBDFE7] text-slate-500/60'} relative`}>
        <div className="flex items-center gap-3">
          ...
          {activeFile && (
            <div className="flex items-center animate-fade-in">
              <div className={`flex items-center gap-2 h-6 transition-colors ${isDark ? 'text-white/70' : 'text-slate-500/60'}`}>
        ...
        <div className="flex items-center gap-3">
          {activeFile && (
            <div className="hidden sm:flex items-center gap-4">
              <div className="flex items-center gap-1.5 h-4">
                <Code2 size={14} />
                <span>
                  LN {cursorPosition.ln}, COL {cursorPosition.col} {selectionCount > 0 && `(${selectionCount} selected)`}
                </span>
              </div>
            </div>
          )}
          {rateLimit && (
            <div className="flex items-center gap-2 h-4">
              ...
              <div className="flex items-center gap-2">
                <div className={`w-20 sm:w-28 h-1 rounded-full ${isDark ? 'bg-slate-800' : 'bg-slate-300'} overflow-hidden`}>
                  <div
                    className={`h-full transition-all ${rateLimit.userRemaining <= 5 ? 'bg-red-500' : rateLimit.userRemaining <= 12 ? 'bg-yellow-500' : 'bg-green-500'}`}
                    style={{ width: `${(rateLimit.userRemaining / rateLimit.userLimit) * 100}%` }}
                  />
                </div>
                <span className={`${rateLimit.userRemaining <= 5 ? 'text-red-500' : rateLimit.userRemaining <= 12 ? 'text-yellow-500' : ''}`}>
                  {rateLimit.userRemaining}/{rateLimit.userLimit}
                </span>
              </div>
            </div>
          )}
